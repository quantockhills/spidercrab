    ...
    int clientId, const std::string& id, const std::string& params)
{
    ...
    // Restore slot source paths from project ext state (persisted across sessions)
    if (m_api.GetProjExtState) {
        char sourcesBuf[65536] = {0};
        int ret = m_api.GetProjExtState(nullptr, "SPIDERCRAB", "slotSources",
            sourcesBuf, (int)sizeof(sourcesBuf));
        if (ret >= 0 && sourcesBuf[0]) {
            m_playtimeState.loadSources(sourcesBuf);
        }
    }
...
// ============================================================
// REAPER Media Explorer library commands
// ============================================================
